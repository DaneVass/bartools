...
plotCellsPerGroup <- function(sc.obj = NULL, 
  ...
  if (order) {
    ...
    bc.tally$Var1 <- factor(bc.tally$Var1, levels = bc.tally$Var1[order(bc.tally$Freq, decreasing = F)])
  } 
  
  # count number of barcodes 
  bc.tally$num.barcodes <- factor(unlist(lapply(strsplit(as.character(bc.tally$Var1), 
                                                         split = ",", perl = T), length)))
  
  # add labelling variable according to defined cell number threshold 
  bc.tally$label <- ifelse(bc.tally$Freq > threshold, as.character(bc.tally$Var1), "")
  ...
  if (plot) {
    p <- ggplot2::ggplot(bc.tally) + 
      ...
      ggplot2::scale_color_manual(values = viridis::viridis(length(unique(bc.tally$num.barcodes)))) +
    ...
    if (label){
      if(rlang::is_installed("ggrepel")) {
        p <- p + ggrepel::geom_text_repel(data = bc.tally, 
                                          ggplot2::aes(x = Var1, y = Freq, color = num.barcodes, label = label),
                                          max.overlaps = 20)  
        return(p)
      } else {
        p <- p + ggplot2::geom_text(data = bc.tally, 
                                    ggplot2::aes(x = Var1, y = Freq, color = num.barcodes, label = label),
                                    nudge_x = -20)  
        return(p)
      }
