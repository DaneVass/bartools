...
plotLibraryDiversity <-
  function(barcodes,
    ...
    barcodes <-
      barcodes[which(barcodes$Raw_count >= cutoff), ] # arbitrary 10 as default
    ...
    p <-
      ggplot2::ggplot(barcodes, ggplot2::aes(y = barcodes$Raw_count, x = seq(1, length(
        rownames(barcodes)
      )))) +
      ggplot2::geom_point(
        stat = "identity",
      ...
      ggplot2::geom_hline(yintercept = stats::median(barcodes$Raw_count),
                          color = "grey40") +
...
plotLibraryCumSum <-
  function(barcodes,
    ...
    p <-
      ggplot2::ggplot(d, ggplot2::aes(y = d$proportion, x = d$barcode)) +
