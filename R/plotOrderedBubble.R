...
plotOrderedBubble <- function(counts.obj,
  ...
  barcodes.proportional$Position <- barcodes.proportional[,orderSample]
  ...
  if(colorDominant){
    ...
    Highbarcodes <- dplyr::filter_all(barcodes.proportional[,1:(ncol(barcodes.proportional)-3)],
  ...
  HighbarcodesLabel <- barcodes.proportional[rownames(Highbarcodes),]
  HighbarcodesLabel <- HighbarcodesLabel[HighbarcodesLabel$Position > 0,]
  ...
  bubble.plot <- ggplot2::ggplot(barcodes.proportional.melted,
    ...
    ggplot2::scale_x_continuous(trans='log10', labels =
                                  HighbarcodesLabel$Barcode,
                                breaks = HighbarcodesLabel$Position,
                                sec.axis = ggplot2::sec_axis(~ . * 1, labels = c(0.0001,0.001,0.01,0.1,1,2,5,10,20,30,40,50),
                                                             breaks = c(0.0001,0.001,0.01,0.1,1,2,5,10,20,30,40,50))) +
    ggplot2::theme_bw() +
    ggplot2::theme(legend.position = "none",
          axis.text.x = ggplot2::element_text(angle=90, vjust=0.5, colour = HighbarcodesLabel$Color, size = 6),
          axis.text.y = ggplot2::element_text(size = 6),
          plot.title = ggplot2::element_text(size=8),
          axis.title = ggplot2::element_text(size= 6))

  suppressWarnings(print(bubble.plot))
