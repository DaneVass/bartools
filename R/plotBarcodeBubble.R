...
#' @export
#' @examples
#' plotBarcodeBubble(test.dge$counts, labels = TRUE, proportion.cutoff = 10)
#' plotBarcodeBubble(test.dge$counts, labels = FALSE, proportion.cutoff = 10)
...
plotBarcodeBubble <-
  function(counts.obj,
    ...
    barcodes.proportional$Position <-
      as.factor(seq(1, length(rownames(counts.obj))))
    ...
    if (labels) {
      ...
      Highbarcodes <-
        barcodes.proportional.melted[barcodes.proportional.melted$Proportion > proportion.cutoff, ]
      # only take unique barcode labels
      HighbarcodeOrdered <-
        unique(Highbarcodes[order(Highbarcodes$Barcode), 1:3])
        ...
        bubble.plot <- ggplot2::ggplot(
          ...
          ggplot2::theme(
            ...
            axis.text.x = ggplot2::element_text(
              angle = 90,
              vjust = 0.5,
              colour = HighbarcodeOrdered$Color,
              size = 5
            ),
