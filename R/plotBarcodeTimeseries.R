...
plotBarcodeTimeseries <-
  function(counts,
           ...
           seed = 5,
           top = 50) {
    barcodes.proportional <- as.data.frame(counts)
    ...
    names(top.bc) <-
      colnames(barcodes.proportional)[-length(colnames(barcodes.proportional))]
    ...
    colors <-
      sample(colors, length(rownames(barcodes.proportional.melted)), replace = TRUE)
    barcodes.proportional.melted$color <- colors

    # inspired by genBaRcode package
    timeseries.plot <- ggplot2::ggplot(
      barcodes.proportional.melted,
      ggplot2::aes_string(
        group = "Barcode",
        fill = "Barcode",
      ...
      ggplot2::scale_x_discrete(breaks = timepoints,
                                labels = timepoints,
                                limits = timepoints) +
      ggplot2::theme(
