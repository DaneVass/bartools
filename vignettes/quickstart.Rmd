...
```{r}
dge.filtered <- thresholdCounts(test.dge, type = "absolute", threshold = 10, min.samps = 3, plot = F)
```
...
```{r}
corrs <- calcReplicateCorr(dge.filtered, group = dge.filtered$samples$Group, corr.thresh = 0.95)
corrs
...
```{r}
plotOrderedBubble(dge.filtered.collapsed$counts, proportion.cutoff = 10, labels = T, orderSample = "Sensitive_TP0_T0_BR1", colorDominant = F)
```

Barcodes that fail to meet a defined abundance threshold in any sample can be greyed out
```{r}
plotOrderedBubble(dge.filtered.collapsed$counts, proportion.cutoff = 10, labels = T, orderSample = "Sensitive_TP0_T0_BR1", colorDominant = T)
```
...
Here barcodes ranked among the top n most abundant within each sample are indicated by an asterix.
This heatmap shows high dose samples are generally distinct from the low dose and vehicle group.

```{r}
plotBarcodeHeatmap(cpm(dge.filtered.collapsed$counts), N = 5, show_bc = T)
```
...
```{r}
plotBarcodeCumSum(dge.filtered.collapsed$counts, sample1 = "Sensitive-TPO", samples = colnames(dge.filtered.collapsed$counts)[1:5])
```
