...
Barcode samples were harvested from the bone marrow of moribund mice and sequenced for in technical replicate replicate.
...
<!-- TODO remove hard-coded file paths or create chunk with eval=FALSE -->
<!-- TODO rename variables, so they don't overwrite test.dge -->
```{r eval=FALSE}
samplesheet <- read.csv(system.file("extdata", "test_sampletable.csv", package = "bartools", mustWork = T), header = T, stringsAsFactors = F)
samplesheet

sampletable <- read.csv("~/Dropbox/PostDoc_PeterMac/Manuscripts/2022_bartools_manuscript/datasets/dose_escalation/dose_escalation_samplesheet.csv", header = T)
dose <- readDGE(files = sampletable, group = sampletable$Run, labels = sampletable$Sample, header = T)
test.dge <- dose

test.dge <- test.dge[,which(test.dge$samples$Biol_Rep == 1)]
test.dge.2 <- test.dge[,which(test.dge$samples$Biol_Rep == 2)]
...
<!-- TODO eval=FALSE, see last chunk-->
<!-- expects column "files" -->
```{r eval=FALSE}
...
<!-- TODO loading data twice  -->
```{r}
data(test.dge)
test.dge
test.dge$samples$Treatment[c(1,2)] <- "Baseline"
test.dge <- test.dge[,order(test.dge$samples$Treatment)]
```
...
We first want to ensure that we are working with clean data. Using the `thresholdCounts()` function we can determine an appropriate
threshold to apply to the data to maximise signal to noise and retain true and informative barcodes. 
<!-- TODO describe more clearly what the objective is  -->
<!-- TODO write function to fileter barcodes that are in all PCR replicates -->
...
<!-- TODO description does not match text -->
```{r}
dge.filtered <- thresholdCounts(test.dge, type = "absolute", threshold = 10, min.samps = 2, plot = F)
```

We then normalise samples to sequencing depth using `normaliseCounts()`
```{r}
...
<!-- TODO y-axis legend needs "log10" label. -->
<!-- TODO describe that barcodes are named after frequency in ref library -->
...
<!-- TODO axis labels -->
<!-- TODO would it make sense to add a pseudocount to 0 before log transformation? Or repalce them with NA? -->
```{r}
...
Note that we expect a very high correlation because these are PCR duplicates of the same barcode pool.
We can easily get the correlation values between replicates

<!-- TODO don't save plot, remove corr.thresh, only relevant if filtering for good or bad -->
```{r}
corrs <- calcReplicateCorr(dge.filtered, group = "Group", corr.thresh = 0.9)
corrs

dat <- as.data.frame(corrs)
p <- ggplot(reshape2::melt(dat, value.name = "Correlation"), aes(x = variable, y = Correlation)) + 
  geom_boxplot(width = 0.4, outlier.size = 0) + 
  geom_jitter(width = 0.02,) +
  #geom_hline(yintercept = 0.95, linetype='dotted', col = 'blue') +
  #annotate("text", x = "corrs", y = 0.9, label = "Correlation Threshold", vjust = -0.5, hjust = -1) +
...
# ggsave("~/Desktop/boxplot.png", width = 3, height = 4)
...
Now that we know our samples are of good quality we can merge the PCR replicate information.
...
<!-- TODO shouldn't the group be re-set? -->
...
<!-- TODO I don't know how this is supposed to look like but columns don't work and what is the time here? -->
```{r}
# plotBarcodeTimeseries(dge.filtered.collapsed[,dge.filtered.collapsed$samples$Treatment %in% c("400nM", "T0", "600nM", "800nM", "1000uM")], top = 10)
...
<!-- TODO color legend title -->
```{r}
plotBarcodePCA(dge.filtered.collapsed, intgroup = "Treatment")
plotBarcodePCA(dge.filtered.collapsed[,dge.filtered.collapsed$samples$Treatment %in% c("Baseline","Vehicle","High_dose")], intgroup = "Treatment")
```
...
<!-- TODO high dose samples are not really distinct from the low dose and vehicle group -->
<!-- TODO Description of parameter N not understandable:
String of sample 1 name (used to ranked barcode abundance) 
could be extended by complex heatmap metadata -->

```{r}
plotBarcodeHeatmap(
  cpm(dge.filtered.collapsed$counts),
  N = 5,
  show_bc = T
)
```
...
These plots show that both top barcodes in the T0 sample were essentially extinguished by the high dose treatment
<!-- TODO Adjust description -->

```{r}
# plotBarcodeBoxplot(dge.filtered.collapsed, barcodes = c(top.bc$`1000nM-7-weeks`,top.bc$`DMSO-8-weeks`), condition = c("Sensitive-TPO", "400nM", "1000nM"))
...
<!-- TODO This output is v annoying in html format -->
```{r}
...
diff.bc$pv <- -log10(diff.bc$PValue)
```


```{r}
EnhancedVolcano(toptable = diff.bc, x = "logFC", y = "PValue", lab = rownames(diff.bc))
```
