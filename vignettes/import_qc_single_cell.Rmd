...
```{r}
counts_path <- "~/Dropbox/PostDoc_PeterMac/Manuscripts/2022_bartools_manuscript/datasets/barcoded_single_cell/BARtab_out/counts/unmapped.counts.tsv"
```

```{r}
counts <- readBartabCounts(counts_path)
counts <- head(counts, 3000)
write.delim(counts2, "~/Dropbox/PostDoc_PeterMac/Scripts/bartools/inst/extdata/test.bartab.counts.tsv")
```
...
```{r}
counts_filtered <- filterBarcodes(counts, umiCountFilter = 2, umiFractionFilter = 0.5)
```
...
```{r}
plotBarcodesPerCell(test.sce@colData, aggregated = T, sep = ",")
```
