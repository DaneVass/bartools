...
plotBarcodeCorrelation(dge.filtered$counts, clustered = T, upper = F, method = "pearson")
plot
...
```{r}
# Pearson correlation, full
plotBarcodeCorrelation(dge.filtered.collapsed$counts, clustered = T, upper = F, method = "pearson")
```
...
```{r}
plotBarcodeTimeseries(counts.obj = dge.filtered.collapsed, 
                      name = "test.data", 
                      top = 50, 
                      seed = 10101)
...
```{r}
plotBarcodePCA(object = dge.filtered.collapsed, 
               intgroup = "Treatment", 
               ntop = 1000)
```
...
```{r}
plotBarcodeHeatmap(
  counts = edgeR::cpm(dge.filtered.collapsed$counts),
  N = 5,
  show_bc = F,
  samples = dge.filtered.collapsed$samples,
  group = "Treatment"
...
```{r}
samples <- which(dge.filtered.collapsed$samples$Treatment == "Vehicle")
...
plotBarcodeBoxplot(dge.obj = dge.filtered.collapsed, 
                   group = "Treatment",
...
```{r}
plotAbundanceLines(dge = dge.filtered.collapsed,
